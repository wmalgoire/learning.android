...
//Filesystem (checkDir, createDir, checkFile, creatFile, removeFile, writeFile, readFile)
  .factory('$cordovaFile', ['$q', function ($q) {
    ...
    return {
      ...
      moveFile: function (filePathFrom, filePathTo) {
          var q = $q.defer();

          getFilesystem().then(
                function (filesystem) {
                    filesystem.root.getFile(filePathFrom, { create: false },
                      // success
                      function (fileEntry) {
                          fileEntry.moveTo(filesystem.root, filePathTo, function () { q.resolve(); }, null);
                      },
                      // error
                      function (error) {
                          q.reject(error);
